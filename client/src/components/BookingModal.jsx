...
import { X, Check, CheckCircle, ArrowRight, ArrowLeft, ShieldCheck, QrCode, Smartphone, Sparkles, Printer, MessageCircle } from 'lucide-react';
import confetti from 'canvas-confetti';
import { useSite } from '../context/SiteContext';

export const BookingModal = ({ isOpen, onClose, initialDate, initialFrame }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    notes: '',
    services: [
      availableServices[0] || { id: 'cand-photo', name: 'Candid & Traditional Photography', price: 24999 }
    ],
  ...
  useEffect(() => {
    if (initialFrame) {
      setFormData(prev => ({ ...prev, frame: initialFrame }));
    }
  }, [initialFrame]);
  ...
  const availableFrames = [
    { id: 'teak-wood', name: 'Classic Teak Wood', price: 1499, dimensions: '12x18 inches' },
    { id: 'canvas-wrap', name: 'Canvas Wrap', price: 1999, dimensions: '16x24 inches' },
    { id: 'acrylic-float', name: 'Acrylic Floating', price: 2499, dimensions: '18x24 inches' },
    { id: 'matte-black', name: 'Matte Black Minimalist', price: 1799, dimensions: '14x20 inches' },
    { id: 'golden-vintage', name: 'Golden Vintage', price: 2199, dimensions: '16x20 inches' }
  ];

  const toggleService = (srv) => {
    const exists = formData.services.some(s => s.id === srv.id);
    if (exists) {
      if (formData.services.length === 1) return; // keep at least one
      setFormData({ ...formData, services: formData.services.filter(s => s.id !== srv.id) });
    } else {
  ...
  const calculateTotal = () => {
    ...
    const frameTotal = formData.frame ? formData.frame.price : 0;
    return servicesTotal + frameTotal;
  };
  ...
  const handleNext = () => {
    if (step === 1) {
      if (!formData.clientName || !formData.email || !formData.phone || !formData.eventDate) {
        alert('Please fill in all required fields (Name, Email, Phone, Event Date)');
        return;
      }
    }
    setStep(prev => prev + 1);
  ...
  const handleSubmitPayment = async () => {
    if (!formData.transactionId) {
      alert('Please enter your UPI Transaction ID or UTR number.');
      return;
    }
  ...
  // UPI deep link & QR target
  const upiId = settings.upiId || 'avstudio.shoot@oksbi';
  const upiQrString = `upi://pay?pa=${upiId}&pn=${encodeURIComponent(settings.studioName || 'AV STUDIO')}&am=${totalAmount}&cu=INR&tn=Booking-${formData.eventType}`;
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: '1.25rem', fontWeight: 800, color: '#fff' }}>
              Book Now <span style={{ color: 'var(--text-dim)', fontSize: '0.95rem', fontWeight: 400 }}>• Step {step} of 5</span>
            </span>
          </div>
        ...
        {/* Step Indicator (Screen 6 in image) */}
        <div style={{
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'space-between',
        ...
        <div style={{ padding: '28px', overflowY: 'auto', flexGrow: 1 }}>
          {/* STEP 1: Personal & Event Information (Screen 6 in image) */}
          {step === 1 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <h3 style={{ fontSize: '1.25rem', fontWeight: 700, color: '#fff' }}>
                1. Personal Information & Event
              </h3>
              ...
              <div>
                ...
                <textarea
                  rows="3"
                  placeholder="Tell us about special requirements, number of guests, or favorite themes..."
          ...
          {step === 2 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h3 style={{ fontSize: '1.25rem', fontWeight: 700, color: '#fff' }}>
                  2. Choose Services & Coverage
                </h3>
                <span style={{ color: 'var(--text-dim)', fontSize: '0.85rem' }}>Select all that you need</span>
              </div>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {availableServices.map((srv) => {
                  const isChecked = formData.services.some(s => s.id === srv.id);
                  return (
                    <div
                      key={srv.id}
              ...
              <div style={{
                ...
                <span style={{ color: 'var(--text-dim)', fontSize: '0.9rem', fontWeight: 600 }}>Services Subtotal</span>
                <span style={{ fontSize: '1.3rem', fontWeight: 800, color: '#ff2a85' }}>
                  ₹{formData.services.reduce((a, b) => a + b.price, 0).toLocaleString('en-IN')}
                </span>
              </div>
            </div>
          )}

          {/* STEP 3: Frame Customization */}
          {step === 3 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h3 style={{ fontSize: '1.25rem', fontWeight: 700, color: '#fff' }}>
                  3. Select a Signature Photo Frame (Optional)
                </h3>
              </div>
              <p style={{ fontSize: '0.9rem', color: 'var(--text-dim)' }}>
                Add an archival museum frame to showcase your event portraits at home, or skip to proceed.
              </p>

              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
                {/* Option to skip / no frame */}
                <div
                  onClick={() => setFormData({ ...formData, frame: null })}
                  style={{
                    padding: '16px',
                    borderRadius: '14px',
                    background: formData.frame === null ? 'rgba(255, 42, 133, 0.15)' : 'rgba(255, 255, 255, 0.03)',
                    border: formData.frame === null ? '1.5px solid #ff2a85' : '1px solid rgba(255, 255, 255, 0.08)',
                    cursor: 'pointer',
                    display: 'flex',
                    flexDirection: 'column',
                    justifyContent: 'center',
                ...
                >
                  ...
                  <div style={{ fontSize: '0.78rem', color: 'var(--text-dim)' }}>
                    Just digital gallery & video
                  </div>
                  <div style={{ fontSize: '0.9rem', fontWeight: 800, color: 'var(--text-muted)', marginTop: '8px' }}>
                    ₹0
                  </div>
                </div>

                {availableFrames.map((frm) => {
                  const isSelected = formData.frame?.id === frm.id;
                  return (
                    <div
                      key={frm.id}
                      onClick={() => setFormData({ ...formData, frame: frm })}
                      style={{
                        padding: '16px',
                        borderRadius: '14px',
                        background: isSelected ? 'rgba(255, 42, 133, 0.15)' : 'rgba(255, 255, 255, 0.03)',
                        border: isSelected ? '1.5px solid #ff2a85' : '1px solid rgba(255, 255, 255, 0.08)',
                        cursor: 'pointer',
                        display: 'flex',
                        flexDirection: 'column',
                        justifyContent: 'space-between',
                    ...
                    >
                      <div>
                        <div style={{ fontWeight: 700, color: '#fff', fontSize: '0.95rem' }}>{frm.name}</div>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-dim)', marginTop: '2px' }}>{frm.dimensions}</div>
                      </div>
                      <div style={{
                        fontFamily: "'Outfit', sans-serif",
                        fontSize: '1.1rem',
                        fontWeight: 800,
                        color: isSelected ? '#ff4d9d' : '#fff',
                        marginTop: '12px'
                      }}>
                        +₹{frm.price.toLocaleString('en-IN')}
                      </div>
                    </div>
                  );
                })}
          ...
          {/* STEP 4: Payment - UPI QR (Screen 6 in image) */}
          {step === 4 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              <div style={{
                ...
                <div style={{ textAlign: 'right', fontSize: '0.82rem', color: 'var(--text-muted)' }}>
                  <div>{formData.eventType} Session</div>
              ...
              {/* Scan & Pay UPI QR Section (Matches Screen 6 in image) */}
              <div style={{
                display: 'grid',
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    <img
                      src={qrCodeApiUrl}
                      alt="AV Studio UPI QR Code"
                      style={{ width: '100%', height: '100%', objectFit: 'contain' }}
                    />
                  </div>

                  <div style={{ textAlign: 'center' }}>
                    <span style={{ fontSize: '0.72rem', color: 'var(--text-dim)', display: 'block' }}>UPI ID</span>
                    <code style={{
                      color: '#ff4d9d',
                      fontWeight: 700,
                      fontSize: '0.92rem',
                ...
                <div>
                  ...
                  <ol style={{ fontSize: '0.84rem', color: 'var(--text-muted)', lineHeight: 1.6, paddingLeft: '20px', marginBottom: '20px' }}>
                    <li>Open {paymentApp} or any UPI app on your phone.</li>
                    <li>Scan the QR code above or pay to UPI ID.</li>
                    <li>Complete the payment of <strong>₹{totalAmount.toLocaleString('en-IN')}</strong>.</li>
                    <li>Copy and enter the 12-digit UPI Transaction ID / UTR below.</li>
                  </ol>
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="e.g. 984019284712 or TXN849102"
          ...
          {/* STEP 5: Confirmation (Screen 6 in image) */}
          {step === 5 && bookingResult && (
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', maxWidth: '500px', margin: '0 auto 24px' }}>
                Thank you, <strong>{bookingResult.clientName}</strong>! Your shoot has been reserved and our team has been notified.
              </p>
              ...
              <div className="glass-panel" style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.88rem' }}>
                  ...
                  <span style={{ color: '#fff', fontWeight: 600 }}>{bookingResult.eventType} ({bookingResult.eventDate})</span>
                </div>

                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.88rem' }}>
                  <span style={{ color: 'var(--text-dim)' }}>Contact</span>
                  <span style={{ color: '#fff' }}>{bookingResult.phone}</span>
                </div>
